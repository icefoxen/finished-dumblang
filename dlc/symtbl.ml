...
(*
let litexpr2arraylitexpr expr =
  match expr with
      Literal( _, lit ) -> 
	let list = makeList len lit in
	  Arraylit( list )
    | _ -> errorAndDie "Blech!  Not an expr!"
*)
...
(* Returns a string representing a type.  This will actually exist
   more fully later.
*)
let rec type2str name =
  match name with
      AtomType( s, i ) -> s
    | FuncType( rettype, args ) -> "function"
    | RefType( t ) -> "ref " ^ (type2str t)
    | ArrayType( arrtype ) -> 
	Printf.sprintf "array: %s" (type2str arrtype) 
    | NoneType -> "None"
;;
...
(* Ummm.... yeah.  Sure, we can hard-code it for now... 
   Returns true if giventype is a subtype of exptype
*)
let isSubtype tbl exptyp giventyp =
...
(* If it's just ints, it's an int array
   If it's ints and floats, or just floats, it's a float array
   If it has an array in it, then all it's values must be arrays
   of the same type.
   Fold hasCompatibleType over it, baby!
   ...doesn't handle arrays, though.
*)
let getArrayLiteralType tbl typelist =
  (*let arrayTypeIsCompatible typ lasttype =
    match typ with
    AtomType( name, size ) -> 
    | FuncType( typetype, args ) ->
    | ArrayType( array, size ) ->
    | NoneType -> errorAndDie "impossible type in array decl!"
    in*)
  let f x y = 
    if isSubtype tbl x y then y 
    else errorAndDie "Impossible type in array decl!"
  in
  let atype = List.fold_left f (List.hd typelist) (List.tl typelist) in
    ArrayType( atype )
;;
...
let isTypeOfArray stbl arr t =
  match t with
      ArrayType( typ ) -> isSubtype stbl t typ
    | _ -> false
;;
